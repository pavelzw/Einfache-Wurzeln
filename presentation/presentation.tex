...
\begin{frame}{Wurzel}
    ...
    nennen wir im Folgenden immer ein Wurzelsystem. \(r\) 
    nennen wir im Folgenden immer eine Wurzel.
\end{frame}
\section{Motivation}
\begin{frame}{Motivation}
    Gegeben \( \mathscr{G} \) eine effektive Spiegelungsgruppe. 

    Kennen bereits das Wurzelsystem \( \Delta \) von 
    \( \mathscr{G} \). \pause 
    
    \( \Delta \) hilft uns bei 
    Klassifikation von \( \mathscr{G} \). \pause
...
\begin{frame}{Definition \( t \)-Basis}
    ...
    \pause
    Im Folgenden ist mit \( \Pi \) immer eine 
    \(t\)-Basis gemeint.
\end{frame}
...
\begin{frame}{Beispiele}
    ...
    \begin{columns}[c]
        \begin{column}{0.7\textwidth}
            ...
            \end{align*}
            \onslide<5->{\centering{}Andere Wahlen für \( \Pi \) nicht 
            möglich.}
        \end{column}
